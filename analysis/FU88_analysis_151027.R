...
hydroxy.probes.CEP.ext <- cbind(hydroxy.probes.CEP, mean.beta.CEP[rownames(hydroxy.probes.CEP),])
hydroxy.probes.CEGX.ext <- cbind(hydroxy.probes.CEGX, mean.beta.CEGX[rownames(hydroxy.probes.CEGX),])

hydroxy.probes.CEP.ext$hmc <- hydroxy.probes.CEP.ext$BS - hydroxy.probes.CEP.ext$oxBS
hydroxy.probes.CEGX.ext$hmc <- hydroxy.probes.CEGX.ext$BS - hydroxy.probes.CEGX.ext$oxBS



#debug, plot a single ceph hyrdoxymeth betas ----------------------------------------
CEP.debug <- c(9,10) #CEPH BS & oxBS
CEP.debug.2 <- c(9,11) # CEPH BS repl1 & CEPH repl2

debug.betas<- getBeta(mset.swan)[,CEP.debug] #Single sample CEPH hydroxy plot
debug.betas.2<- getBeta(mset.swan)[,CEP.debug.2] #Single CEPH BS-BS to check spread for smalpe wo oxBS involved

debug.hmc <- debug.betas[,1] - debug.betas[,2]
debug.hmc.2 <- debug.betas.2[,1] - debug.betas.2[,2]

densityplot(debug.hmc) 
densityplot(debug.hmc.2)

#load data from Yanaras project to compare CEPH 
baseDir.Yanara <- '~/projects/FU88/NJ-0636_FU82_150430_IDAT'
targets.Yanara <- read.450k.sheet(baseDir.Yanara)
RGset.Yanara <- read.450k.exp(targets = targets.Yanara)
detP.Yanara <- detectionP(RGset.Yanara)
failed.Yanara <- detP.Yanara > 0.01
maxFail.Yanara <- 0
mset.Yanara <- preprocessRaw(RGset.Yanara)
#drop all rows with a failed probe in any sample
mset.Yanara <- mset.Yanara[rowSums(failed.Yanara) <= maxFail.Yanara, ]
mset.swan.Yanara <-preprocessSWAN(RGset.Yanara, mSet = mset.Yanara)
